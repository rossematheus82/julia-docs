/**
 * Municípios do IBGE — chave = código IBGE de 6 dígitos (sem o dígito
 * verificador), que é o formato devolvido pelo campo `codigo_municipio` da
 * API do CNES/DataSUS.
 *
 * Tabela local (dados públicos do IBGE) em vez de consulta on-line: evita
 * depender de mais uma API externa no cadastro de estabelecimento.
 *
 * ARQUIVO GERADO — não edite à mão. Para atualizar, regere as linhas no
 * formato `codigo6|Nome|UF` a partir da API de Localidades do IBGE:
 * https://servicosdados.ibge.gov.br/api/v1/localidades/municipios
 */
...
240120|Arês|RN
240130|Augusto Severo (Campo Grande)|RN
...
240510|Jandaíra|RN
240520|Janduís|RN
240530|Januário Cicco (Boa Saúde)|RN
240540|Japi|RN
...
let cache: Map<string, { nome: string; uf: string }> | null = null

function tabela() {
  if (cache) return cache
  const mapa = new Map<string, { nome: string; uf: string }>()
  for (const linha of TABELA.trim().split('\n')) {
    const [codigo, nome, uf] = linha.split('|')
    if (codigo && nome) mapa.set(codigo, { nome, uf })
  }
  cache = mapa
  return mapa
}
...
/**
 * Converte o código IBGE do município (6 ou 7 dígitos) em nome e UF.
 * Retorna `null` quando o código não existe na tabela.
 */
export function municipioPorCodigoIbge(
  codigo: string | number | null | undefined,
): { nome: string; uf: string } | null {
  const digitos = String(codigo ?? '').replace(/\D/g, '')
  if (digitos.length < 6) return null
  return tabela().get(digitos.slice(0, 6)) ?? null
}
